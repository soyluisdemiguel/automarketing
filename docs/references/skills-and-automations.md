...
| `visibility-refresh` | Daily for tracked queries | Search visibility sources, MCP discovery checks, app metadata | New `visibility_observations` and rank deltas | New or changed visibility signals recorded for tracked surfaces | Stop when provider quotas or app limits are exceeded |
